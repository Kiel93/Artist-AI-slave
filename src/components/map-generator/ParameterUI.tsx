        ...
        if (exportGrid && gridLevels['1']) {
          ...
          for (let row = 0; row < h * 3; row++) {
            ...
            for (let col = 0; col < w * 3; col++) {
              ...
              const isLand = layer1[tileRow] && layer1[tileRow][tileCol] && layer1[tileRow][tileCol].isLand;
            ...
            for (let col = 0; col < grid[row].length; col++) {
              ...
              if (exportGround && layer >= 0 && (cell.tileId || layer === 0)) {
                ...
                if (decal) {
                  ...
                  const sliceIdx = groundAsset?.slices?.findIndex(s => s.name === ('Ground_' + cell.tileId));
                  const slice = sliceIdx !== undefined && sliceIdx !== -1 ? groundAsset!.slices[sliceIdx] : undefined;
          ...
          if (objAsset) {
            ...
            if (!mapConfig.assetDefinitions.find((d: any) => d.id === cleanPrefabName)) {
              mapConfig.assetDefinitions.push({
                ...
                baseTiles: (objAsset.baseTiles || [{ lx: 0, ly: 0 }]).map((t: any) => ({ lx: -t.ly, ly: -t.lx }))
              });
            }
          }
        }
...
`;
      ...
      if (exportGround) {
        ...
        // Removed foam export from ground, moved fully to Ocean export
